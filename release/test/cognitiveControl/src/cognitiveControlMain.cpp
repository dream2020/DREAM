...
int main(int argc, char * argv[])
{
   ...
   ResourceFinder rf;
   rf.setVerbose(true);
   rf.setDefaultConfigFile("cognitiveControl.ini");              // can be overridden by --from parameter
   rf.setDefaultContext("components/cognitiveControl/config");   // can be overridden by --context parameter
   rf.configure("DREAM_ROOT", argc, argv);                                   // environment variable with root of configuration path
 
   /* run the module: runModule() calls configure first and, if successful, it then runs */

   cognitiveControl.runModule(rf);
