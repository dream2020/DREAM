 /** @file actuationDriverController.h 
  *
  * Version 1.0
  * 
...
#ifndef ACTUATIONDRIVERCONTROLLER
#define ACTUATIONDRIVERCONTROLLER
...
public:
	/* class methods */
	ActuationDriverController(ActuationDriverYarpInterface *yarpInterface);
   //Need to add the methods calling/being called by the YarpInterface
	void testSelection(int stepId, int actionId, std::string argument);
	void testSelection(int stepId, int actionId, int argument);
	void testSelection(int stepId, int actionId);
	void goNextStep();
	void test();
	void waitms(int millisec);
	void simulateExecution(int step, int action);
	void getFromSandtrayCommand(std::string message);
	ActuationDriverYarpInterface* _yarpInterface;
	bool _touchExpectedGrab;
	bool _touchExpectedDrop;
};
