...
int main(int argc, char *argv[])
{
	...
	/* create the module */
	NaoInterfaceDriver naoInterfaceDriver;

	/* prepare and configure the resource finder */
	yarp::os::ResourceFinder rf;
	rf.setVerbose(true);
	rf.setDefaultConfigFile("naoInterfaceDriver.ini");
	rf.setDefaultContext("components/naoInterfaceDriver/config");
	rf.configure("DREAM_ROOT",argc, argv);

	/* run the module: runModule() calls configure first, and if successful, it then runs */
	naoInterfaceDriver.runModule(rf);
