...
int main(int argc, char *argv[])
{
	...
	/* create the module */
	DeliberativeDriver deliberativeDriver;

	/* prepare and configure the resource finder */
	yarp::os::ResourceFinder rf;
	rf.setVerbose(true);
	rf.setDefaultConfigFile("deliberativeDriver.ini");
	rf.setDefaultContext("components/deliberativeDriver/config");
	rf.configure("DREAM_ROOT",argc, argv);

	/* run the module: runModule() calls configure first, and if successful, it then runs */
	deliberativeDriver.runModule(rf);
