...
int main(int argc, char *argv[])
{
	...
	/* create the module */
	ActuationSubsystem actuationSubsystem;

	/* prepare and configure the resource finder */
	yarp::os::ResourceFinder rf;
	rf.setVerbose(true);
	rf.setDefaultConfigFile("actuationSubsystem.ini");
	rf.setDefaultContext("components/actuationSubsystem/config");
	rf.configure("DREAM_ROOT",argc, argv);

	/* run the module: runModule() calls configure first, and if successful, it then runs */
	actuationSubsystem.runModule(rf);
